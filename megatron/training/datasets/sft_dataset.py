...
class SFTDatasetConfig(GPTDatasetConfig):
    """Configuration for SFT datasets, including optional mock data settings."""
...
class SFTDataset(MegatronDataset):
    ...
    @staticmethod
    def build_low_level_dataset(dataset_path: str, config: SFTDatasetConfig) -> LowLevelDataset:
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        for conversation in split_conversations:
            ...
            original_seq_lengths.append(
                max(min(unpadded_end, pack_length) - padded_start, 0)
            )
        ...
        cu_seqlens_unpadded = torch.tensor(
            [0, *np.cumsum(original_seq_lengths).tolist()], dtype=torch.int32
        )
        ...
        padded_cu_seqlens_unpadded = torch.full(
            (pack_length + 1,), cu_seqlens_unpadded[-1], dtype=torch.int32
        )
        padded_cu_seqlens_unpadded[: cu_seqlens_unpadded.numel()] = cu_seqlens_unpadded
        ...
        return {
            ...
            'cu_seqlens_unpadded': padded_cu_seqlens_unpadded,
...
class MockSFTDataset(SFTDataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        tokens_list = tokens.tolist()
        if not tokens_list or tokens_list[-1] != eod:
            tokens_list.append(eod)
        original_seq_len = min(len(tokens_list), pack_length)
        ...
        return {
            ...
            'cu_seqlens_unpadded': torch.tensor(
                [0, original_seq_len], dtype=torch.int32
            ),
            'max_seqlen': max_seqlen,
        }
